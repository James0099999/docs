...
<ParamField body="transactionId" type="string" required>
  Unique reference of this cancel transaction created in Scorpio Play. Use this to enforce idempotency.
</ParamField>

<ParamField body="referenceId" type="string" required>
  The `transactionId` of the original bet transaction being cancelled. Use this to look up the bet you need to reverse in your system.
</ParamField>
...
<ParamField body="amount" type="number" required>
  The refund amount to credit back to the player's balance. This matches the original bet amount.
</ParamField>
...
<Warning>
  Your response must be returned within **4 seconds**. If the request times out or returns an error, Scorpio Play will automatically retry the request up to **2 more times**.
</Warning>
