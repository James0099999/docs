...
The `betwin` callback is sent when a player places a bet. Your server must deduct the bet amount from the player's balance and return the updated balance. This operation is idempotent — if your wallet receives more than one request with the same `transactionId`, you must register the transaction only once.
